...
def highlight_mentions(text):
    def replace(match):
        ...
        return f'<a href="{url_for("user.user_profile", user_id=user_id)}" class="mention text-blue-400 hover:underline">@{escape(username)}</a>'
...
def process_mentions(content):
    ...
    def repl(match):
        ...
        if user:
            return f'<a href="{url_for("user.user_profile", user_id=user.id)}" class="mention text-blue-400 hover:underline">@{escape(username)}</a>'
